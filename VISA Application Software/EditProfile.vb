    ...
    Private Sub EditProfile_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            Using con As New MySqlConnection(My.Settings.conStr)
                con.Open()
                Dim query = "SELECT `FName`, `LName`, `Email`, `PNumber`, `UserName`, `DoB`, `Password` FROM `signup` where `Id` = '" & Login1.id & "'"
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        If ValidateForm() Then
            ...
            Try
                Using con As New MySqlConnection(My.Settings.conStr)
                    con.Open()
                    Dim query = "UPDATE `signup` SET `FName`=@FName,`LName`=@LName,`Email`=@Email,`PNumber`=@PNumber,`DoB`=@DoB,`Password`=@Password WHERE `UserName`=@UserName;"
                    Using cmd As New MySqlCommand(query, con)
